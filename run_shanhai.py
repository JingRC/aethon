...
穷奇=暴躁/嘴臭/吃素/香菜死敌。陈默=面瘫/社恐/"……"就算一句完整的话。

漫画对话铁律（来自专业编剧教程）：
1. 每格<=1说话人、<=2对话框。对话框里是对话——不是信息、不是列表、不是选项。
2. 角色说角色的话——穷奇骂骂咧咧像工地大叔。陈默几乎不说话。让角色自己开口。
3. 不直说情绪——不说"我很生气"，说"操"或者摔碗或者不说话。用动作和潜台词。
4. 每句对话必须做功——要么推剧情、要么展性格、要么改关系。废话删掉。
5. 口语化——不说"此次考试失利深感愧疚"，说"考砸了丢死人了"。
6. SFX中文（啪/轰/嗡/咚/咔/唰）。narration<=2。dialogue和narration中文。image_prompt唯一英文。

坏例子（不要写）：
  dialogue:"怕。但已下单。先确认收货。" ← 三个意思挤一句，像清单
  dialogue:"你小费50" ← 像翻译软件，不像人话
好例子（应该写）：
...
2. 陈默几乎不说话。非要说话就1-5个字。\"……\"和\"嗯\"也算对话。不是高冷。是社恐+真没什么好说的。
3. 穷奇说话粗暴接地气。像工地大叔骂街。带脏字但不恶心。穷奇对话≤20字。
4. 全章narration≤3处。
5. 笑点公式：穷奇暴躁→陈默面瘫→穷奇更暴躁。
6. 结尾必须是钩子。
7. 画面英文，以上述前缀开头。
8. 不连续3页同一layout。
只返回JSON。不解释。""",
